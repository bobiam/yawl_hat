/*yawl hat - sending yaw data via software serial using a pile of spaghetti to control blinky lights on a 
 * unreasonably overcomplicated hat.  Because art.  Or science.  Or something.
 * If you use this code, I'd like to offer you my sincere congratudolences because you're clearly trying to do something
 * that's pretty silly.  This code could be keeping drones in the sky or moving telescopes, but I'm using it to 
 * light leds on a hat.
 */
...
float min_pitch, min_yaw, min_roll, max_pitch, max_yaw, max_roll;
...
//note - only handles one wrap.
int subtractWithWrap(int val1, int val2, int wrapAt)
{
  if(val1 - val2 >= 0)
  {
    //no need to wrap.
    ...
    //handle wrap.
    return wrapAt + (val1-val2);
  }
}

void setup()
{
  //hold 2 seconds for LED power-up.
  delay(2000);  
...
//    Serial.print(" qz = "); Serial.println(quat[3]); 
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    //where do we want to draw the light? - triggering "lock yaw" will eventually set this value.
    int degrees_locked_at = 30;

    int leds_per_degree, led_on;
    leds_per_degree = 360/num_leds;
    int cleaned_int_yaw;
    cleaned_int_yaw = floor(yaw+180);
    if(cleaned_int_yaw > 359)
    {
      cleaned_int_yaw = 0;
    }      

    put_light_at = subtractWithWrap(cleaned_int_yaw, degrees_locked_at, 360);
    /*
    Serial.print(cleaned_int_yaw);
    Serial.print("  ");
    Serial.print(degrees_locked_at);
    Serial.print("  ");
    Serial.println(put_light_at);

            */
    
    byte transmit_value;
    transmit_value = put_light_at/leds_per_degree;    

    leds[transmit_value] = CRGB::White;
    FastLED.show();
    fadeall();     
    /*
    if(just_led){
      ...
      byte transmit_value;
      transmit_value = cleaned_int_yaw/leds_per_degree;
/*
      if(mySerial.available()){
        ...
        if(in == 'G')
        {
          ...
          mySerial.print("Here comes some data ");
          mySerial.print(transmit_value);
        }else{
          Serial.print("Char in was ");
          Serial.println(in);
        }
      }
    ...
    */
      ...
      Serial.print(min_yaw, 2);
      Serial.print(", ");
      Serial.print(max_yaw, 2);
      Serial.print(", ");
      Serial.print(min_pitch, 2);
      Serial.print(", ");
      Serial.print(max_pitch, 2);
      Serial.print(", ");
      Serial.print(min_roll, 2);
      Serial.print(", ");
      Serial.println(max_roll, 2);
      /*
    }
